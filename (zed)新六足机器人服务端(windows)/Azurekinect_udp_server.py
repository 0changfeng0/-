...
def data_process(msg):
        ...
        dx=int((msg[0])*1000.0)
        dy=int((msg[1])*1000.0)
        yaw=int((msg[2])*1000.0*15)
        xyheight=90
        speed=int(msg[4]*1000.0)

        msg = numpy.array(
            (ctr0, ctr1, ctr2, dx, dx >> 8, dy, dy >> 8, yaw, yaw >> 8, xyheight, xyheight >> 8, speed, speed >> 8),
            dtype=numpy.uint8)
        msg = (msg.tostring())
        msg = msg.hex()
        # print(msg)
        ...
        # print(type(msg1))
        # msg1=numpy.array((ctr0, ctr1, ctr2, dx, dx >> 8, dy, dy >> 8, yaw,yaw>>8,xyheight,xyheight>>8,speed,speed>>8), dtype=numpy.uint8)
        crc1=calculateCRC(str(msg))

        new_msg = numpy.array((ctr0, ctr1, ctr2, dx, dx >> 8, dy, dy >> 8, yaw,yaw>>8,xyheight,xyheight>>8,speed,speed>>8,crc1,crc1>>8), dtype=numpy.uint8)
...
class Azure_udp_server():
    ...
    def receive_image(self,control_msg):
        try:
            ...
            new_msg=data_process(control_msg)

            # new_msg='10'
            # self.server.sendto(bytes.fromhex(str(new_msg)), address1)




            # self.server.sendto(bytearray.fromhex(new_msg), address1)
            ...
            color_image = cv2.resize(imgdecode1, (1280, 720))
            cv2.circle(color_image, (90, 200), 60, (255, 125, 0), 2, 0)
            cv2.line(color_image, (90, 200), (int(90+control_msg[0]*60*control_msg[3]), int(200-control_msg[1]*60*control_msg[3])), (0, 255, 0), 2)
            cv2.line(color_image, (30, 270),
                     (150, 270),
                     (255, 125, 0), 2)
            cv2.line(color_image, (90,270),
                     (int(90+control_msg[2]*60),270),
                     (0, 255, 0), 2)

            cv2.putText(color_image, 'dx = %1.1f'%(control_msg[0]), (40, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
            cv2.putText(color_image, 'dy = %1.1f' % (control_msg[1]), (40, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (0, 255, 0), 1)
            cv2.putText(color_image, 'yaw = %1.1f' % (control_msg[2]), (40, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (0, 255, 0), 1)
            cv2.putText(color_image, 'step = %1.1f' % (control_msg[3]), (40, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (0, 255, 0), 1)
            cv2.putText(color_image, 'speed = %1.1f' % (control_msg[4]), (40, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (0, 255, 0), 1)
            x1=int(170)
            y1=int(260-control_msg[4]*120)
            cv2.rectangle(color_image, (x1, y1), (175, 260), (0, 255, 0), -1)


            # cv2.namedWindow('color image', cv2.WINDOW_NORMAL)
            cv2.imshow('color image', color_image)  # 显示图像
            ...
            print(seconds)
            print(new_msg)
            self.server.sendto(new_msg, address1)
            # self.server.send(bytes(str(int(fps)), encoding='utf-8'))
...
if __name__ == '__main__':
    ...
    msg=[0.0,0.0,0.0,1.0,1.0]
    new_msg=data_process(msg)
    print(new_msg)
    ...
    # while True:
    #         k=azure_server.receive_image(msg)
    #         if k == 27:
    #             break
